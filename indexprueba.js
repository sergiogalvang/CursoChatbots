...
//global.imagenes = "https://us-central1-curso1-88e63.cloudfunctions.net/curso/imagenes/"    //Cargar Imagenes del directorio
...
server.post("/curso", (req, res) => {
    ...
    let opciones = LibreriaSGG.reducirAOcho(["chiste", "Consejo", "Noticias", "Mi Equipo"]);
    ...
    //6.   Ejercicio de Menu
        ...
        resultado = LibreriaSGG.respuestaBasica("Te muestro algunas cosas que se hacer:");
    }
    ...
    //7.   Recomendar ordenador
        ...
        if (!tipopc) {
            ...
            opciones = ["Menú"];    //Hacemos que las sugerencias no aparezcan para que la recomendacion no este al final
        }
    }
    ...
    //10.   Informacion de ponentes
        ...
        try {                                           //si conoce el ponente
            ...
            opciones = opciones = LibreriaSGG.reducirAOcho(arListaPonentes.slice());
            opciones.unshift("Menú");   //agraga el menu al inicio
        }
...
const local = true;         // Variable para que ejecutar en el ordenador = true

if (local == true) {
    server.listen((process.env.PORT || 8000), () => { console.log("Servidor funcionando"); })
} else {
    exports.curso = functions.https.onRequest(server); //Para FIrebase en la nube = false
}